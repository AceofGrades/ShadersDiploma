	...
	SubShader
	{
		Pass
		{
			...
			float4 frag(v2f i) : color
			{
				...
				float l = 1 - ce;
				float3 ax = 1;
				ax *= paper.b; ax = lerp(float3(0,0,0), ax * _Value3 *1.5,1);
				float gg = lerp(l - paper.g, 0, 1 - _Value5);
				ax = lerp(ax, float3(0,0,0),gg);
				paper.rgb = float3(paper.r, paper.r, paper.r);
				paper.rgb *= float3(0.695,0.496,0.3125)*1.2;
				paper = lerp(paper, _PColor2, _Value6);
				paper = lerp(paper, _PColor, ax * _Value3);
				float pg = gg *0.2;
				paper -= pg *0.5;
